...
class ConfigurableAggregateManager[A <: AggregateLike](behaviorCons: A#Id => Behavior[A], val passivationStrategy: PassivationStrategy)
...
object ConfigurableAggregateManager {
  ...
  def props[A <: AggregateLike](behaviorCons: A#Id => Behavior[A], passivationConfigPath: Option[String]) = {
    Props(new ConfigurableAggregateManager(behaviorCons, PassivationStrategy(passivationConfigPath)))
  }
}